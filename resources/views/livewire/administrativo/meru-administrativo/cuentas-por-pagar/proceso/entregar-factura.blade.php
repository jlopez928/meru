<div wire:init="cargar_emit" x-data="">
    ...
    <x-form method="post" action="{{ route('cuentasxpagar.proceso.recepfactura.entregar', $recepfactura->id) }}">
     <x-card>
          <x-slot name="header">
              <h3 class="card-title text-bold">Entregar Factura</h3>
          </x-slot>
          <x-slot name="body">
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <div  class="tab-pane fade" id="devolucion" role="tabpanel" aria-labelledby="devolucion-tab">
                    @include('administrativo/meru_administrativo/cuentasxpagar/proceso/recep_factura/partials/_deventrega')
                </div>
            </div>
          </x-slot>
          <x-slot name="footer">
              <x-input type="submit" class="btn btn-sm btn-primary text-bold float-right"  value="Entregar" />
          </x-slot>
      </x-card>
  </x-form>
  ...
  <script type="text/javascript">
    ...
    window.livewire.on('enableEntrega', param => {
        $('#fec_entrega').attr('readonly', false);
    });
  </script>
@endpush
