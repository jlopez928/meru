...
<div class="row col-12">
	<div class="form-group col-2 offset-2">
		...
		<x-input type="text" name="ano_pro" class="form-control form-control-sm text-center" value="{{ $disminucion->ano_pro }}" disabled/>
	</div>

	<div class="form-group col-2">
		<x-label for="xnro_mod">Código</x-label>
		<x-input type="text" name="xnro_mod" class="form-control form-control-sm text-center"  value="{{ $disminucion->xnro_mod }}" disabled />
	</div>

    <div class="form-group col-2">
		<x-label for="nro_mod">Número</x-label>
		<x-input type="text" name="nro_mod" class="form-control form-control-sm text-center"  value="{{ $disminucion->nro_mod }}" disabled />
	</div>

    <div class="form-group col-2">
		<x-label for="num_doc">Solicitud</x-label>
		<x-input type="text" name="num_doc" class="form-control form-control-sm text-center"  value="{{ $disminucion->num_doc }}" disabled />
	</div>
</div>
...
<div class="row col-12">
	<div class="form-group col-3 offset-3">
		...
		<x-input type="text" name="fec_tra" class="form-control form-control-sm text-center" value="{{ \Carbon\Carbon::parse($disminucion->fec_tra)->format('d/m/Y'); }}" disabled />
	</div>

	<div class="form-group col-3">
		<x-label for="fec_sta">Fecha Estado</x-label>
		<x-input type="text" name="fec_sta" class="form-control form-control-sm text-center" value="{{ \Carbon\Carbon::parse($disminucion->fec_sta)->format('d/m/Y'); }}" disabled />
	</div>
</div>
...
<div class="row col-12">
	<div class="form-group col-8 offset-2">
		...
		<x-input type="text" name="concepto" class="form-control-sm"  maxlength="500" value="{{ $disminucion->concepto }}" disabled/>
	</div>
</div>
...
<div class="row col-12">
	<div class="form-group col-8 offset-2">
		...
		<textarea id="justificacion" name="justificacion" class="form-control" maxlength="300" cols="50" rows="5" title="Indique Justificación" disabled>{{ $disminucion->justificacion }}</textarea>
	</div>
</div>
...
<div class="row col-12">
	<div class="form-group col-3 offset-2">
		...
		<span name="estado">{{ Str::replace('_', ' ', $disminucion->sta_reg->name) }}</span>
	</div>
</div>
	...
	<div class="card-body">
		...
		<div class="row col-12" style="paddin-right:0px !important;">
			<div class="form-group row col-12">
				...
				<div class="col-2">
					<x-input name="total_ced" class="text-right" value="{{ number_format($disminucion->totalCedentes(), 2, ',', '.') }}" disabled/>
				</div>
			</div>
		</div>
	</div>
