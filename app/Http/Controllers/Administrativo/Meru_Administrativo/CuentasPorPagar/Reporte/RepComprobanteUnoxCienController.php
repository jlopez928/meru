...
class RepComprobanteUnoxCienController extends Controller
{

    use ReportFpdf;
    use PreMovimientos;

    public function print_generar_unoxcien(Solpago $solicititudpago)
    ...
    {   $PUNTODECIMAL=',';
		...
		$pdf->Ln();
		$pdf->Cell(200,4,'UNO POR CIEN',0,0,'C',0);
		$pdf->Header();
        $pdf->SetY($Y + 15);
		$pdf->SetFont('Times','',9);
		...
		$pdf->SetX(28);
		$pdf->Cell(48,4,'Concepto de Retencion ','L,R,T',0,'C',1);
		$pdf->SetX(76);
		$pdf->Cell(10,4,'% Ret.','L,R,T',0,'C',1);
		$pdf->SetX(86);
		$pdf->Cell(26,4,'Nro Factura','L,R,T',0,'C',1);
		$pdf->SetX(112);
		$pdf->Cell(34,4,'Nro Control','L,R,T',0,'C',1);
        ...
        $pdf->Cell(48,4,'Impuesto Retenido','L,R,T',0,'C',1);
		$pdf->Ln();

        //ojppj revisar esto
        // dd($datos_prov[0]->comprobantesretencion[0]->ComprobantesDetRetencionNC);
		/*if (strpos($datos_prov[0]->comprobantesretencion[0]->ComprobantesDetRetencionNC->id_des, 'NC') === false)
			$encabe = 'Débito';
		else
			$encabe = 'Crédito';*/
            $encabe = 'Débito';
        ...
        $pdf->SetFillColor(255, 255, 255);
        $pdf->SetFont('Times','B',6);
        $comprobante=$datos_prov[0]->comprobantesretenciontc[0]->comprobantesdetretencion;
        $pdf->SetX(10);
        ...
        $pdf->SetX(86);
        $pdf->Cell(26,5,$comprobante->nro_factura,1,0,'C',1);
        $pdf->SetX(112);
        $pdf->Cell(34,5,$comprobante->nro_control,1,0,'C',1);
        ...
        $pdf->SetX(162);
        $pdf->Cell(48,5,$this->formatNumber($comprobante->mto_retencion,2,",",$PUNTODECIMAL,"(") ,1,0,'C',1);
        $pdf->Ln();
