...
public function calculaCostoTotal ($valor){
    ...
    if (!empty($this->codigo)){
        if (!empty($this->cod_ger)) {
            ...
            $this->des_con=ConceptoContrato::query()
            ->where('sta_reg', '1')
            ...
            if (!empty($this->des_con)){
                ...
                $EstructuraPresupuestaria=ConceptosContratoDet::select(DB::raw("'1' AS gasto"),
                                                                    'ppg.cod_par','ppg.cod_gen', 'ppg.cod_esp', 'ppg.cod_sub',
                                                                    'ppg.des_con',
                                                                    'ppg.cta_gasto',
                                                                    DB::raw("CASE WHEN op_conceptos_contrato_det.cod_com IN ('07.01.02.01') THEN 2 ELSE CAST(substr(g.centro_costo, 1, 2) AS INTEGER) END AS tip_cod"),
                                                                    DB::raw("CASE WHEN op_conceptos_contrato_det.cod_com IN ('07.01.02.01') THEN 1 ELSE CAST(substr(g.centro_costo, 4, 2) AS INTEGER) END AS cod_pryacc"),
                                                                    DB::raw("CASE WHEN op_conceptos_contrato_det.cod_com IN ('07.01.02.01') THEN 0 ELSE CAST(substr(g.centro_costo, 7, 2) AS INTEGER) END AS cod_obj"),
                                                                    DB::raw("CASE WHEN op_conceptos_contrato_det.cod_com IN ('07.01.02.01') THEN 1 ELSE CAST(substr(g.centro_costo, 10, 2) AS INTEGER) END AS gerencia"),
                                                                    DB::raw("CASE WHEN op_conceptos_contrato_det.cod_com IN ('07.01.02.01') THEN 0 ELSE CAST(substr(g.centro_costo, 13, 2) AS INTEGER) END AS unidad"),
                                                                    )
                                                        ->join('op_conceptos_contrato AS oc', 'oc.cod_con', '=', 'op_conceptos_contrato_det.cod_con')
                                                        ->join('gerencias AS g', 'g.cod_ger', '=',  DB::raw($this->cod_ger))
                                                        ->join('pre_partidasgastos AS ppg',
                                                            function($join) {
                                                                $join->on('ppg.cod_par', '=', 'op_conceptos_contrato_det.cod_par')
                                                                ->on('ppg.cod_gen', '=', 'op_conceptos_contrato_det.cod_gen')
                                                                ->on('ppg.cod_esp', '=', 'op_conceptos_contrato_det.cod_esp')
                                                                ->on('ppg.cod_sub', '=', 'op_conceptos_contrato_det.cod_sub');
                                                            })
                                                            ->where('op_conceptos_contrato_det.cod_con','=',$this->codigo)
                                                            ->where('oc.sta_reg','=','1')
                ...
                if(!$EstructuraPresupuestaria->isEmpty()){
                    ...
                    $this->emit('swal:alert', [
                        'tipo'    => 'warning',
                        'titulo'  => 'Error',
                        'mensaje' => 'Error. Servicio Seleccionado No Existo o No Tiene Partida Asociada'
                    ]);
                    $this->emit('alert', ['tab' => '#detalle-tab','det' => $this->detallegasto]);
                }
            }else{
...
//-------------------------------------------------------------------------
    ...
    {
        return view('livewire.administrativo.meru-administrativo.contratos.proceso.tab-certificacion-contrato');
    }
}
