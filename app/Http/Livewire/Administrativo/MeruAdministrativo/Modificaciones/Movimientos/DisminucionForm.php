...
class DisminucionForm extends Component
{
    public $disminucion;
    public $anoPro;
    public $xnroMod;
    ...
    public function mount($disminucion)
    {
        $this->traspaso = $disminucion;
        $this->cedente  = [
            'tip_cod'    => '',
            'cod_pryacc' => '',
            'cod_obj'    => '',
        ...
        if ($this->traspaso->id) {
            $this->anoPro  = $disminucion->ano_pro;
            $this->xnroMod = $disminucion->xnro_mod;
            $this->nroMod  = $disminucion->nro_mod;
            $this->numDoc  = $disminucion->num_doc;
            $this->fecTra  = \Carbon\Carbon::parse($disminucion->fec_tra)->format('d/m/Y');
            $this->fecSta  = \Carbon\Carbon::parse($disminucion->fec_sta)->format('d/m/Y');
            $this->concepto      = $disminucion->concepto;
            $this->justificacion = $disminucion->justificacion;
            $this->estructurasCedentes = $disminucion->estructurasCedentes(); // Revisar modelo
            $this->totCed = number_format($disminucion->totalCedentes(), 2, ',', '.');
        } else {
    ...
    {
        return view('livewire.administrativo.meru-administrativo.modificaciones.movimientos.disminucion-form');
    }
}